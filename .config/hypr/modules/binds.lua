...
local rofiClipboard =
	'pkill rofi || cliphist list | rofi -dmenu -display-columns 2 -p " 󰅍 " -theme ~/.config/rofi/style-2.rasi | cliphist decode | wl-copy'
local rofiWallpaper = "pkill rofi || ~/.config/hypr/scripts/wallpaper-picker.sh"
local rofiEmoji =
	'pkill rofi || rofimoji --files all --action type copy --prompt " ⬢ " --selector-args="-theme ~/.config/rofi/style-2.rasi"'
...
hl.bind(
	mainMod .. " + K",
	hl.dsp.exec_cmd("pkill wvkbd-mobintl || wvkbd-mobintl -L 400 -H 400 -l full,arabic --landscape-layers full,arabic")
)
...
hl.bind(
	mainMod .. " + SHIFT + F",
	hl.dsp.exec_cmd(
		'pkill nwg-dock-hyprla || nwg-dock-hyprland -x -mb 2 -i 33 -w 5 -c "rofi -show drun -theme ~/.config/rofi/style-1.rasi" -lp start -ico /usr/share/pixmaps/archlinux-logo.svg'
	)
)
